...
class TestCoursesWithMock(TestCase):
    ...
    def test_page_open_detail(self):
        ...
        with open('mainapp/fixtures/005_feedback_list_1.bin', "rb") as input_file, mock.patch(
                'django.core.cache.cache.get'
        ) as mocked_cache:
            mocked_cache.return_value = pickle.load(input_file)
            result = self.client.get(path)
            self.assertEqual(result.status_code, HTTPStatus.OK)
...
class TestTaskMailSend(TestCase):
    ...
    def test_mail_send(self):
        ...
        mainapp_tasks.send_feedback_to_email(message_text, user_obj.id)
...
class TestNewsSelenium(StaticLiveServerTestCase):
    ...
    def test_pick_color(self):
        ...
        try:
            ...
            with open('var/screenshots/001_navbar_el_screenshot.png', 'wb') as output_file:
                output_file.write(navbar_el.screenshot_as_png)
            raise
